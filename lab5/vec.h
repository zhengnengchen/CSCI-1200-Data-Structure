...
template <class T> void Vec<T>::resize(size_type n, const T& fill_in_value) {
    if (n <= m_size) {
        m_size = n;
        if (m_size <= m_alloc / 2) {
            T* new_data = new T[m_alloc / 2];
            for (size_type i = 0; i < m_alloc / 2; ++i) new_data[i] = m_data[i];
            delete [] m_data;
            m_data = new_data;
            m_alloc /= 2;
        }
...
template <class T> unsigned int remove_matching_elements(Vec<T>& v,
                                                         const T& element) {
    typename Vec<T>::iterator v_itr = v.begin();
...
template <class T> void Vec<T>::print() const {
    std::cout << "m_size: " << m_size << "; m_alloc: " << m_alloc << "; m_data:";
    // Corrected
