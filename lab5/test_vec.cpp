...
int main() {
  ...
  cout << endl;
    ...
    unsigned int c = remove_matching_elements(v1, 100);
    cout << "Deleted " << c << " items from v1. Now v1 is:" << endl;
    v1.print();
    
    // [0, 1, 2, 3]
    Vec<int> v4(4, 0);
    for (int i = 0; i < 4; ++i) v4[i] = i;
    c = remove_matching_elements(v4, 4);
    cout << "Deleted " << c << " items from v1. Now v1 is:" << endl;
    v4.print();
    
    // Remove 2
    c = remove_matching_elements(v4, 1);
    cout << "Deleted " << c << " items from v1. Now v1 is:" << endl;
    v4.print();
    
    // Remove first element
    c = remove_matching_elements(v4, 0);
    cout << "Deleted " << c << " items from v1. Now v1 is:" << endl;
    v4.print();
    
    // Remove last element
    c = remove_matching_elements(v4, 3);
    cout << "Deleted " << c << " items from v1. Now v1 is:" << endl;
    v4.print();
    
    // Remove the only element
    c = remove_matching_elements(v4, 2);
    cout << "Deleted " << c << " items from v1. Now v1 is:" << endl;
    v4.print();
