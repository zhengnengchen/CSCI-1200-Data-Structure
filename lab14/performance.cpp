...
void priority_queue_test(const std::string* input_data, int input_count, const std::string &operation, std::string *output_data, int &output_count) {
    ...
    if (operation == "sort") {
        ...
        std::cerr << "Cannot do this operation on priority_queue" << std::endl;
        exit(0);
    } else if (operation == "mode") {
        std::string mode, current = pq.top();
        int mode_count = 0, freq = 0;
        for (int i = 0; i < input_count; ++i) {
            if (current == pq.top()) freq++;
            else {
                current = pq.top();
                freq = 1;
            }
            if (mode_count < freq && mode != current) {
                mode = current;
                mode_count = freq;
            }
            pq.pop();
