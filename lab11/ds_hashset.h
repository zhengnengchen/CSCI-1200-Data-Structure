...
public:
    ...
    iterator begin() {
        ...
        for (unsigned int i = 0; i < m_table.size(); ++i) {
            if (m_table[i].begin() != m_table[i].end())
...
private:
    ...
    void resize_table(unsigned int new_size) {
        // Implement this function for Lab 12, Checkpoint 2, Part 2
        ...
        for (unsigned int i = 0; i < m_table.size(); ++i) {
            for (hash_list_itr itr = m_table[i].begin();
                 itr != m_table[i].end();
                 ++itr) {
                //  Compute the hash value and then the table index
                ...
                new_table[index].push_front(*itr);
            }
        }
