...
public:
    ...
    unsigned int operator() ( std::string const& key ) const {
        ...
        // Checkpoint 1, part 2: a trivial hash function that is guaranteed to
        // have many, many collisions.
//        return 0;
        
        // Checkpoint 1, part 2: a hash function that is not terrible, but will
        // unfortunately always place anagrams (words with the same letters, but
        // rearranged) in the same bin
//        unsigned int hash = 1315423911;
//        for(unsigned int i = 0; i < key.length(); i++)
//            hash |= ((hash << 5) + key[i] + (hash >> 2));
//        return hash;
...
int main() {
    ...
    // ---------------------------------
    // CODE TO TEST CHECKPOINT 1, PART 2
    insert_result = set1.insert( std::string("bca") );
    assert( insert_result.second );
    
    insert_result = set1.insert( std::string("cba") );
    assert( insert_result.second );
    
    insert_result = set1.insert( std::string("pppuy") );
    assert( insert_result.second );
    
    insert_result = set1.insert( std::string("zabre") );
    assert( insert_result.second );
    // ---------------------------------
